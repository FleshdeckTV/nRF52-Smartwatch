...
byte SPIClass::transferD(uint8_t data)
{
  const nrfx_spim_xfer_desc_t xfer_desc = NRFX_SPIM_XFER_TX(&data, 1);
...
uint16_t SPIClass::transfer16D(uint16_t data) {
	
  MSB_16_SET(data, data);
  
  const nrfx_spim_xfer_desc_t xfer_desc = NRFX_SPIM_SINGLE_XFER(&data, 2, NULL, 0);
  nrfx_spim_xfer(&_spim, &xfer_desc, 0);
...
#define SPI_32MHZ_INTERFACE 0
  ...
  #define _SPI_DEV    NRF_SPIM2 // 32 Mhz
  #define _SPI1_DEV   NRF_SPIM3

#elif SPI_32MHZ_INTERFACE == 1
  #define _SPI_DEV    NRF_SPIM3
  #define _SPI1_DEV   NRF_SPIM2 // 32 Mhz
